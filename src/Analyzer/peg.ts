...
function peg$parse(input: string, options?: ParseOptions) {
  ...
  const peg$c0 = function(a: any): any { return a.flat().filter( ( v: any ) => v instanceof AnalyzerToken ) };
  const peg$c1 = "(";
  const peg$c2 = peg$literalExpectation("(", false);
  const peg$c3 = /^[,)]/;
  const peg$c4 = peg$classExpectation([",", ")"], false, false);
  const peg$c5 = function(a: any, b: any): any { return new AnalyzerToken( a, b, text(), location().start.line, location().start.column ) };
  const peg$c6 = /^[ \t\r\n]/;
