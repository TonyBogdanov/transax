...
function peg$parse(input: string, options?: ParseOptions) {
  ...
  const peg$c0 = function(a: any): any { return a.flat().filter( v => v instanceof Token ) };
  const peg$c1 = "(";
  const peg$c2 = peg$literalExpectation("(", false);
  const peg$c3 = /^[,)]/;
  const peg$c4 = peg$classExpectation([",", ")"], false, false);
  const peg$c5 = function(a: any, b: any): any { return new Token( a, b, text(), location().start.line, location().start.column ) };
  const peg$c6 = /^[ \t\r\n]/;
  ...
  const peg$c22 = function(a: any): any { return a.map( ( v: any ) => "\\'" === v ? v[1] : v ).join( '' ) };
  const peg$c23 = "\"";
  ...
  const peg$c29 = function(a: any): any { return a.map( ( v: any ) => '\\"' === v ? v[1] : v ).join( '' ) };
  const peg$c30 = "`";
  ...
  const peg$c38 = function(a: any): any { return a.map( ( [ , v ]: any ) => "\\`" === v ? v[1] : v ).join( '' ) };
...
export class Token {
    ...
    constructor( name: string, key: string, text: string, line: number, column: number ) {
        this.name = name;
        this.key = key;
        this.text = text;
        this.line = line;
...
export class Options {
    ...
    constructor( values: Object = {} ) {
        'source' in values && this.setSource( values.source );
        'names' in values && this.setNames( values.names );
        'verbose' in values && this.setVerbose( values.verbose );
    }

    setSource( value: any ): Options {
...
export class Analyzer {
    ...
    skip( token: Token, message: string ): void {
        if ( !this.options.verbose ) {
            return;
        }

        console.warn( `[WARNING] Skipping: ${ token.text } at line: ${ token.line }, column: ${
            token.column }, because ${ message }${ this.options.source ? ` in ${ this.options.source }` : '' }.` );
    }

    analyze( code: string ): Token[] {
        const result: Token[] = [];
        for ( const token of parse( code ) ) {
            if ( -1 === this.options.names.indexOf( token.name ) ) {
                this.skip( token, `"${ token.name }" is not in the list of configured names` );
                continue;
            }
