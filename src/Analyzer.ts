...
function peg$parse(input: string, options?: ParseOptions) {
  ...
  const peg$c0 = function(a: any): any { return a.flat().filter( ( v: any ) => v instanceof Token ) };
  const peg$c1 = "(";
...
export class Options {
    ...
    constructor( values: Object = {} ) {
        if ( 'names' in values ) {
            if ( !Array.isArray( values.names ) || 0 < values.names.filter( v => 'string' !== typeof v ).length ) {
                throw new Error( 'Options.names must be an array of strings.' );
            }
