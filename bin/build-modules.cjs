...
const run = async ( target, type, ext ) => {
    ...
    await Promise.all( glob.sync( resolve( __dirname, '..', target, '**/*.js' ) )
        .map( path => rename( path, path.substring( 0, path.length - extname( path ).length ) + ext ) ) );

    await Promise.all( glob.sync( resolve( __dirname, '..', target, '**/*' + ext ) ).map( async path => {
